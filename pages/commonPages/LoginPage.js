...
class Login {
    ...
    elements = {
        ...
        PolicyHolderField: () => this.page.frameLocator('iframe[id="loginIframe"]').locator("(//*[@id='IdentityNumber'])"),
        Captcha: () => this.page.frameLocator('iframe[id="loginIframe"]').locator('#captchaCodeForMBIRegister'),
        loginBtnInForm: () => this.page.frameLocator('iframe[id="loginIframe"]').locator('//*[@type="submit"]'),
        password: () => this.page.frameLocator('iframe[id="loginIframe"]').locator('#Password'),
        Otp: () => this.page.locator('.auth-form-otp'),
        OTPInput: () => this.page.frameLocator('iframe[id="loginIframe"]').locator('#OTPInput'),
        VerifyBtn: () => this.page.frameLocator('iframe[id="loginIframe"]').locator('#btnVerify'),
        SchemeCaptchaErrorMessage: () => this.page.locator('#SchemeCaptchaErrorMessage'),
        errorMsgFromcaptcha: () => this.page.frameLocator('iframe[id="loginIframe"]').getByText('رمز التحقق المدخل غير صحيح'),

        //signup
        signupTap: () => this.page.frameLocator('iframe[id="loginIframe"]').locator("#btnsignup"),
        mobileNumberInput: () => this.page.frameLocator('iframe[id="loginIframe"]').locator("#MobileNumber"),
        emailInput: () => this.page.frameLocator('iframe[id="loginIframe"]').locator("#Email"),
        passwordInput: () => this.page.frameLocator('iframe[id="loginIframe"]').locator("#Password"),
        wrongPassVaildationMsg: () => this.page.frameLocator('iframe[id="loginIframe"]').locator("#Password-error"),
        CaptchaErrorMsgInSignup: () => this.page.frameLocator('iframe[id="loginIframe"]').locator(".validation-summary-errors"),
    ...
    async entercaptchaForm() {
        for (let attempt = 1; attempt <= 10; attempt++) {
            ...
            await expect(this.page.frameLocator('iframe[id="loginIframe"]').locator("#CaptchaImageForMBIRegister")).toHaveJSProperty('complete', true);
            await expect(this.page.frameLocator('iframe[id="loginIframe"]').locator("#CaptchaImageForMBIRegister")).not.toHaveJSProperty('naturalWidth', 0);


            const captchaElement = await this.page.frameLocator('iframe[id="loginIframe"]').locator("#CaptchaImageForMBIRegister");
            ...
            // }
            await this.elements.loginBtnInForm().click()
            await this.page.waitForTimeout(10000); // Adjust this timeout based on your app's behavior
    ...
    async fillOtp() {
        ...
        const iframeElementHandle = await this.page.waitForSelector('iframe[id="loginIframe"]'); // Replace with the actual iframe selector
